...
function rhs = create_rhs_hofd(q, q_ref, grid_obj, form_method, visc, tol)
    nel = grid_obj.nel;
    ngl = grid_obj.ngl;
    nvar = size(q,1);
    rhs = zeros(size(q));
    ...
    for ie=1:nel
        ...
        for i=1:ngl
            ...
            if (idry)
                ...
                weights = fdweights(xgl(i),xgl(wnodes),1);
                b_e = sum(weights.*q_ref(1,wnodes,ie));
                r_e = sum(weights.*q(1,wnodes,ie));
                %u_e = sum(weights.*q(2,wnodes,ie));
                %q(2,wnodes,ie).^2./q(1,wnodes,ie)
                %t_e = sum(weights.*safe_div(q(2,wnodes,ie).^2, q(1,wnodes,ie), 0.0, tol));
            ...
            vu      = visc*safe_div(u_i*abs(u_i)*9.81^(7/3),r_i^(10/3),0.0,10-8);
