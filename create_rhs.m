...
function rhs = create_rhs(q, q_ref, q_bc, grid_obj, time, form_method, visc, tol)
    nel = grid_obj.nel;
    ngl = grid_obj.ngl;
    nvar = size(q,1);
    rhs = zeros(size(q));
    ...
    for ie=1:nel
        for i=1:ngl
            ...
            for j=1:ngl
                ...
                t_e = t_e + h_e*safe_div(q(2,j,ie)*q(2,j,ie),q(1,j,ie),0.0);
            end
            ...
            % calculate the flux vector
            f_r     = u_x;
            %f_u     = 2*u_x*u_i/r_i - r_x*(u_i/r_i)^2 + r_i*s_x;
            %f_u     = 2*u_x*u_i/r_i - r_x*(u_i/r_i)^2 + r_i*r_x;
            f_u     = t_x + r_i*r_x;
            
            % de-singularization
            %if (r_i <= tol); f_u = r_i*r_x; end
